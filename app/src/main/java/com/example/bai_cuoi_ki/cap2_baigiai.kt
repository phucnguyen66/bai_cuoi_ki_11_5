...
class cap2_baigiai : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        fun handleAnswers(kq: Int) {
            ...
            // Hàm tạo giá trị ngẫu nhiên an toàn, tránh tràn Int
            fun generateFakeAnswers(kq: Int): List<Int> {
                val fakeAnswers = mutableSetOf<Int>()
                while (fakeAnswers.size < 3) {
                    val delta = Random.nextInt(5, 31)
                    val sign = if (Random.nextBoolean()) 1 else -1
                    val fake = kq + (sign * delta)
                    if (fake != kq && fake >= 0) fakeAnswers.add(fake)
                }
                return fakeAnswers.toList()
            }
            val answers = (generateFakeAnswers(kq) + kq).shuffled()


            button1.text = "A:   ${answers[0]}"
            button2.text = "B:   ${answers[1]}"
            button3.text = "C:   ${answers[2]}"
            button4.text = "D:   ${answers[3]}"
